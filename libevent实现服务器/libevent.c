...
void dir_html_init(char *dir_buf,int sock_fd)
{
	...
	while(dirp=readdir(dir))
	{
		...
		{
			sprintf(buf,"<li><a href=%chttp://10.20.155.205:%d/%s/%s%c>%s</a>\r\n</li>",'"',PORT,s,dirp->d_name,'"',dirp->d_name);
		}
...
void read_connect(evutil_socket_t fd ,short event,void *arg)
{
    char buf[1500]="";
...
void new_connect(evutil_socket_t fd ,short event,void *arg)
{
    ...
    cfd_event=event_new(base,cfd,EV_READ|EV_PERSIST,read_connect,base);
    event_add(cfd_event,NULL);
}
...
int main(int argc, char const *argv[])
{
    ...
    struct event *lfd_event=event_new(base,lfd,EV_READ|EV_PERSIST,new_connect,base); 
    event_add(lfd_event,NULL);
    event_base_dispatch(base);
    event_base_free(base);
    event_free(lfd_event);
    event_free(cfd_event);
    return 0;
}
